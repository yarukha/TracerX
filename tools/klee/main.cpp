...
cl::opt<std::string> EntryPoint("entry-point", cl::desc("Consider the function with the given name as the entrypoint"),
                                cl::init("main"));
...
cl::opt<bool> WritePCs("write-pcs", cl::desc("Write .pc files for each test case"));
...
cl::opt<LibcType> Libc("libc", cl::desc("Choose libc version (none by default)."),
                       cl::values(clEnumValN(NoLibc, "none", "Don't link in a libc"),
                                  clEnumValN(KleeLibc, "klee", "Link in klee libc"),
                                  clEnumValN(UcLibc, "uclibc", "Link in uclibc (adapted for klee)"), clEnumValEnd),
                       cl::init(NoLibc));

cl::opt<bool>
WithPOSIXRuntime("posix-runtime",
                 cl::desc("Link with POSIX runtime.  Options that can be passed as arguments to the programs are: "
                          "--sym-arg <max-len>  --sym-args <min-argvs> <max-argvs> <max-len> + file model options"),
                 cl::init(false));
...
cl::opt<std::string> OutputDir("output-dir", cl::desc("Directory to write results in (defaults to klee-out-N)"),
                               cl::init(""));

cl::opt<bool> ReplayKeepSymbolic("replay-keep-symbolic", cl::desc("Replay the test cases only by asserting "
                                                                  "the bytes, not necessarily making them concrete."));

cl::list<std::string> ReplayKTestFile("replay-ktest-file", cl::desc("Specify a ktest file to use for replay"),
                                      cl::value_desc("ktest file"));

cl::list<std::string> ReplayKTestDir("replay-ktest-dir", cl::desc("Specify a directory to replay ktest files from"),
                                     cl::value_desc("output directory"));

cl::opt<std::string> ReplayPathFile("replay-path", cl::desc("Specify a path file to replay"),
...
cl::list<std::string> LinkLibraries("link-llvm-lib", cl::desc("Link the given libraries before execution"),
...
cl::opt<unsigned> StopAfterNTests("stop-after-n-tests",
                                  cl::desc("Stop execution after generating the given number of tests.  Extra tests "
                                           "corresponding to partially explored paths will also be dumped."),
                                  cl::init(0));

cl::opt<bool> Watchdog("watchdog", cl::desc("Use a watchdog process to enforce --max-time."), cl::init(0));

cl::OptionCategory TestCompCat("XML Patch options");

cl::opt<bool> WriteXMLTests("write-xml-tests", llvm::cl::desc("Write XML-formated tests"), llvm::cl::init(false),
                            cl::cat(TestCompCat));

cl::opt<std::string> TCOrig("tc-orig", llvm::cl::desc("Test-Comp original file"), llvm::cl::cat(TestCompCat));

cl::opt<std::string> TCHash("tc-hash", llvm::cl::desc("Test-Comp hash sum of original file"),
...
private:
  ...
  void writeTestCaseKTest(const std::vector<std::pair<std::string, std::vector<unsigned char> > > &out, unsigned id);

  void writeTestCaseXML(bool isError, const std::vector<std::pair<std::string, std::vector<unsigned char> > > &out,
                        unsigned id);
...
public:
  ...
  unsigned getTotalBranchingDepthOnExitTermination() { return m_totalBranchingDepthOnExitTermination; }
  unsigned getTotalInstructionsDepthOnExitTermination() { return m_totalInstructionsDepthOnExitTermination; }
  ...
  unsigned getTotalBranchingDepthOnEarlyTermination() { return m_totalBranchingDepthOnEarlyTermination; }
  unsigned getTotalInstructionsDepthOnEarlyTermination() { return m_totalInstructionsDepthOnEarlyTermination; }
  ...
  unsigned getTotalBranchingDepthOnErrorTermination() { return m_totalBranchingDepthOnErrorTermination; }
  unsigned getTotalInstructionsDepthOnErrorTermination() { return m_totalInstructionsDepthOnErrorTermination; }
  unsigned getTotalInstructionPathsExploredOnSubsumption() { return m_totalInstructionsDepthOnSubsumption; }
  ...
  unsigned getTotalBranchingDepthOnSubsumption() { return m_totalBranchingDepthOnSubsumption; }
  ...
  void assignSubsumptionStats(std::string currentStats) { m_subsumptionStats = currentStats; }
...
KleeHandler::KleeHandler(int argc, char **argv)
    : m_interpreter(0), m_pathWriter(0), m_symPathWriter(0), m_infoFile(0), m_outputDirectory(), m_testIndex(0),
      m_pathsExplored(0), m_totalBranchingDepthOnExitTermination(0), m_totalInstructionsDepthOnExitTermination(0),
      m_totalBranchingDepthOnEarlyTermination(0), m_totalInstructionsDepthOnEarlyTermination(0),
      m_totalBranchingDepthOnErrorTermination(0), m_totalInstructionsDepthOnErrorTermination(0),
      m_totalBranchingDepthOnSubsumption(0), m_totalInstructionsDepthOnSubsumption(0), m_subsumptionTermination(0),
      m_subsumptionTerminationTest(0), m_earlyTermination(0), m_earlyTerminationTest(0), m_errorTermination(0),
      m_errorTerminationTest(0), m_exitTermination(0), m_exitTerminationTest(0), m_otherTermination(0), m_argc(argc),
      m_argv(argv) {
...
void KleeHandler::processTestCase(const ExecutionState &state, const char *errorMessage, const char *errorSuffix) {
  ...
  if (!NoOutput) {
    ...
    if (WriteCov) {
      ...
      for (std::map<const std::string *, std::set<unsigned> >::iterator it = cov.begin(), ie = cov.end(); it != ie;
           ++it) {
...
void KleeHandler::getKTestFilesInDir(std::string directoryPath, std::vector<std::string> &results) {
#if LLVM_VERSION_CODE < LLVM_VERSION(3, 5)
  error_code ec;
#else
  std::error_code ec;
#endif
  for (llvm::sys::fs::directory_iterator i(directoryPath, ec), e; i != e && !ec; i.increment(ec)) {
    std::string f = (*i).path();
...
std::string KleeHandler::getRunTimeLibraryPath(const char *argv0) {
  ...
  if (strlen(KLEE_INSTALL_BIN_DIR) != 0 && strlen(KLEE_INSTALL_RUNTIME_DIR) != 0 &&
...
static int initEnv(Module *mainModule) {
  ...
  */
  ...
  Function *initEnvFn = cast<Function>(mainModule->getOrInsertFunction(
      "klee_init_env", Type::getVoidTy(getGlobalContext()), argcPtr->getType(), argvPtr->getType(), NULL));
  assert(initEnvFn);
...
#else
  Instruction *initEnvCall = CallInst::Create(initEnvFn, args.begin(), args.end(), "", firstInst);
#endif
...
static llvm::Module *linkWithUclibc(llvm::Module *mainModule, StringRef libDir) {
  ...
  mainModule->getOrInsertFunction(
      "__uClibc_main", FunctionType::get(Type::getVoidTy(getGlobalContext()), std::vector<LLVM_TYPE_Q Type *>(), true));

  // force various imports
  if (WithPOSIXRuntime) {
    LLVM_TYPE_Q llvm::Type *i8Ty = Type::getInt8Ty(getGlobalContext());
    mainModule->getOrInsertFunction("realpath", PointerType::getUnqual(i8Ty), PointerType::getUnqual(i8Ty),
                                    PointerType::getUnqual(i8Ty), NULL);
    mainModule->getOrInsertFunction("getutent", PointerType::getUnqual(i8Ty), NULL);
    mainModule->getOrInsertFunction("__fgetc_unlocked", Type::getInt32Ty(getGlobalContext()),
                                    PointerType::getUnqual(i8Ty), NULL);
    mainModule->getOrInsertFunction("__fputc_unlocked", Type::getInt32Ty(getGlobalContext()),
                                    Type::getInt32Ty(getGlobalContext()), PointerType::getUnqual(i8Ty), NULL);
  }
...
int main(int argc, char **argv, char **envp) {
  ...
  if (WriteXMLTests) {
    ...
    *meta_file << "\t<architecture>" << mainModule[0].getDataLayout() << "bit</architecture>\n";
    std::stringstream t;
  ...
  if (!ReplayKTestDir.empty() || !ReplayKTestFile.empty()) {
    ...
    for (std::vector<std::string>::iterator it = ReplayKTestDir.begin(), ie = ReplayKTestDir.end(); it != ie; ++it)
      KleeHandler::getKTestFilesInDir(*it, kTestFiles);
    std::vector<KTest *> kTests;
    for (std::vector<std::string>::iterator it = kTestFiles.begin(), ie = kTestFiles.end(); it != ie; ++it) {
    ...
    for (std::vector<KTest *>::iterator it = kTests.begin(), ie = kTests.end(); it != ie; ++it) {
      ...
      llvm::errs() << "KLEE: replaying: " << *it << " (" << kTest_numBytes(out) << " bytes)"
                   << " (" << ++i << "/" << kTestFiles.size() << ")\n";
    ...
    for (std::vector<std::string>::iterator it = SeedOutDir.begin(), ie = SeedOutDir.end(); it != ie; ++it) {
      ...
      KleeHandler::getKTestFilesInDir(*it, kTestFiles);
      for (std::vector<std::string>::iterator it2 = kTestFiles.begin(), ie = kTestFiles.end(); it2 != ie; ++it2) {
  ...
  if (INTERPOLATION_ENABLED) {
    stats << "KLEE: done:     average branching depth of completed paths = "
          << (double)(handler->getTotalBranchingDepthOnExitTermination() +
                 ...
                 (double)(handler->getExitTermination() + handler->getEarlyTermination() +
                          handler->getErrorTermination()) << "\n";
    if (handler->getSubsumptionTermination() == 0.0) {
      stats << "KLEE: done:     average branching depth of subsumed paths = " << 0 << "\n";
    } else {
      stats << "KLEE: done:     average branching depth of subsumed paths = "
            << (double)handler->getTotalBranchingDepthOnSubsumption() / (double)handler->getSubsumptionTermination()
            << "\n";
    }
  }
  ...
  if (INTERPOLATION_ENABLED)
    stats << "KLEE: done:     subsumed paths = " << handler->getSubsumptionTermination() << "\n";
...
#ifdef ENABLE_Z3
  if (SubsumedTest)
    stats << "KLEE: done:     subsumed tests = " << handler->getSubsumptionTerminationTest() << "\n";
#endif
  stats << "KLEE: done:     error tests = " << handler->getErrorTerminationTest() << "\n";
  stats << "KLEE: done:     program exit tests = " << handler->getExitTerminationTest() << "\n";
